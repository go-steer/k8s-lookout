...
| `gateway` | leading (sustained state) | `Gateway`/`HTTPRoute` status conditions via a dynamic informer, discovery-gated on the Gateway API CRDs (post-M5 #168; the expiry-source dynamic/unstructured, no-new-dependency precedent) | the Gateway-API sibling of `ingress` (GKE steers most users to the Gateway API, whose failures never surface as ingress-gce `Sync`/`Translate` events): `Programmed=False` sustained past a grace window (`gateway.programming_failed`), `Accepted`/`ResolvedRefs=False` on a Gateway/listener/HTTPRoute parent (`gateway.route_rejected`). Shares the NEG half with `ingress` (`ingress.neg_failed` already fires for Gateway-backed Services — same NEG controller) |
