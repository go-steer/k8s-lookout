...
(metrics.k8s.io + kubelet stats), `degradation`, `expiry`, and `token-burn`.
...
| `stab drift` | `field-sentinel` | Out-of-band drift vs GitOps manager via `managedFields`. | **Respecced:** `managedFields` yields the *manager string* (`kubectl-edit`), never a user identity — v2's `user=` field requires audit logs. Ships as manager-level detection; identity enrichment via a Cloud Audit Logs query pack later. |
