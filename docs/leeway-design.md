...
> **Seven of the eight are in the v1 ledger as of 2026-09-20, not all eight.**
...
> producer. `leeway.contract_violated`, `leeway.placement_drift`, `leeway.baseline_breach`
> (Phase 5, when Tier C's learned baselines turned on) and all four `leeway.rank_*`
> kinds (Phase 6) are there. `leeway.domain_unavailable` is the one holdout: it is
> raised by the source rather than by a verdict, which is Phase 7. That the names
> were settled here is what made each of those additions mechanical — the commitment
> was made once, in this table, and the ledger caught up to it seven times without
> a rename.
...
capacity (triggered when max/min domain capacity ratio > 1.25, configurable with
`--topology-capacity-ratio`).

The trigger is a **default, not a rule**, and three carve-outs make that precise:
...
| **7 — Nodes** (1.5 wks) | Node-group subjects, capacity weighting, `leeway.domain_unavailable`. **Capacity weighting shipped 2026-09-21 (#468)**: `Weighting` gained an `Auto` zero value so that a declared `Equal` and an undeclared weighting stop being the same thing, every `NodeView` now carries both allocatable dimensions (the trigger has to read CPU to *choose* the weighting, so projecting one resource during eligibility was a chicken-and-egg), and §5.1 resolution writes the resolved weighting back onto the intent with an evidence line quoting the ratio. **This phase amended §7.2** — see the three carve-outs there; the pod-count one was found by breaking two §12 fixtures, and the `minDomains` one would have hidden every padded subject's missing domain | Node-pool imbalance detected and attributed. **The §12 corpus gained a heterogeneous-capacity fixture, 2026-09-21**: ten replicas sitting `[8 1 1]` across zones that are 64, 8 and 8 cores are quiet under the trigger and breach at ρ = 0.4 without it |
