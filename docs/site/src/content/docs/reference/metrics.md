...
| `lookout_runner_up` | gauge | — | 1 while this cluster's watch loop is running, 0 otherwise. Always carries the cluster label; in a multi-cluster process (issue #208) one series per watched cluster reports that runner's liveness independently.", unit: " |
| `lookout_runner_restarts_total` | counter | — | Total in-process restarts of this cluster's runner by the supervisor after it exited while the process stayed up (multi-cluster fate isolation, issue #208). Stays zero in the single-cluster default, where a runner exit ends the process and the kubelet owns restart.", unit: " |
