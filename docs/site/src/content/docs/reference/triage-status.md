...
description: "Write (or read back) the §9.4 triage-status record for an incident — diagnosis, action taken, and your severity judgment — so health scans stop reporting it as a fresh unknown and the sentinel stops re-paging followups; the incident playbooks' closing move."
---
...
| --- | --- | --- | --- |
| `--store` | string | — | path to the sentinel's SQLite store (its --store file). Required: triage-status records live in the sentinel's --store SQLite file (§9.4); see docs/triage-status-write-design.md |
| `--fingerprint` | string | — | the incident-class fingerprint from the inject payload or store row (§8, sha256:…). Required to write; to read, this or --resource selects the record(s) |
...
| `--status` | string | — | triage state to record: investigating\|triaged\|actioned\|escalated (resolved is written by the sentinel's §7.4 recovery flip, never by agents). Empty = read mode: print the current record(s) instead of writing |
