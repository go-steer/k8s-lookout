...
| `saturation` | `pods.metrics.k8s.io` get/list and `nodes/proxy` get — a metrics API must exist (on kind, install metrics-server with `--kubelet-insecure-tls`; the probe fails loudly without it, by design). If the kubelet stats endpoint is unreachable at runtime, the source logs once and skips the PVC dimension. |
