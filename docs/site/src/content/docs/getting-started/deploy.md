...
kubectl apply -k "github.com/go-steer/k8s-lookout/deploy?ref=v0.24.0"
```
...
The chart version tracks the release, minus the `v` — chart `0.24.0`
deploys `v0.24.0`. There is no compatibility matrix to consult because
...
```sh
cosign verify ghcr.io/go-steer/charts/lookout:0.24.0 \
