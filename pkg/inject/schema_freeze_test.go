...
func TestSchemaV1_KindInventory(t *testing.T) {
	...
	// The inventory is complete: 12 cross-cutting + 36 source kinds
	...
	// autoscaling.* +2 in #131, gateway.* +2 in #168 — additive-only,
	// ledger + docs updated in the same changes).
	if len(shippedKinds) != 48 {
		t.Errorf("shipped kind inventory has %d kinds, want 48 — a kind shipped (or was removed) without updating the v1 ledger and docs/signal-schema-v1.md", len(shippedKinds))
	}
}
